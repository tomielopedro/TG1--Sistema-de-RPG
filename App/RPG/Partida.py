...
class Partida:
    """
    Classe que representa uma PARTIDA (composta por uma batalha) realizada entre personagens em uma ARENA.
    Atributos:
        id (str): Identificador único da partida. Gerado com `uuid.uuid4().hex`.
        descricao (str): Descrição textual da partida (ex: "Batalha Total", "Duelo 1x1").
        logs (List[LogCombate]): Lista de eventos de combate, logs de (ataques, habilidades, etc.) ocorridos durante a partida.
        vencedor (Optional[str]): Nome do personagem vencedor da partida, dado atualizado ao ser definido.
    Métodos:
